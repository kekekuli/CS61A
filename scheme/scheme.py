...
def eval_all(expressions, env):
    ...
    value = scheme_eval(expressions.first, env) # change this line
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        operands = expressions.rest.map(lambda x: scheme_eval(x, env))
        if len(operands) != 1:
            raise SchemeError("Incorrect form")
        env.define(target, operands.first) 
        return target
        # END PROBLEM 5
        ...
        fn_object = do_lambda_form(expr, env)
        env.define(fn_name, fn_object)
        return fn_name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    if scheme_symbolp(value) or scheme_numberp(value):
        return value 
    else:
...
def make_let_frame(bindings, env):
    ...
    def helper(bindings):
        ...
        if not scheme_symbolp(name):
            raise SchemeError("Incorrec name for arguments")
        if name in mm:
            raise SchemeError("Repeated argument name")
        mm.append(name)
