...
def g_iter(n):
    ...
    while curr < n:
        ...
        last_two = last_one
        last_one = value
        value = last_one + 2 * last_two + 3 * last_three
    return value
...
def check(curr, last):
    ...
    if curr % 10 > last:
        return False
    return check(curr // 10, curr % 10)

def missing_digits_helper(curr, last):
    if curr == 0:
        return 0 
    if curr % 10 == last:
        return missing_digits_helper(curr // 10, curr % 10)
    else:
        return missing_digits_helper(curr // 10, curr % 10) + last - curr % 10 - 1
...
def count_change_helper(n, m):
    ...
    return count_change_helper(n - m, m) + count_change_helper(n, m * 2)
